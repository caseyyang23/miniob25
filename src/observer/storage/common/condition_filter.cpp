...
{
  LOG_WARN("DefaultConditionFilter does not support expression-based conditions anymore. table=%s", table.name());
  (void)condition;
...
RC CompositeConditionFilter::init(Table &table, const ConditionSqlNode *conditions, int condition_num)
{
  if (condition_num == 0 || conditions == nullptr) {
   ...
   LOG_WARN("CompositeConditionFilter does not support expression-based conditions anymore. table=%s", table.name());
  return RC::UNIMPLEMENTED;
}
