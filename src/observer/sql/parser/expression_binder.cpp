    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  const bool                     is_unary   = arithmetic_expr->arithmetic_type() == ArithmeticExpr::Type::NEGATIVE;
  ...
  if (right_expr != nullptr) {
    ...
    LOG_WARN("right child of arithmetic expression(type=%d) is null", static_cast<int>(arithmetic_expr->arithmetic_type()));
    return RC::INVALID_ARGUMENT;
  }
